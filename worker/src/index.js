...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      if (path === '/' || path === '/health') {
        response = json({ ok: true, service: 'cost-bank-api', version: '2026.06.15-activity' });
        ...
        await requireStudent(request, env, ctx);
        response = json({ ok: true, serverTime: nowIso() });
        ...
        const auth = await requireStudent(request, env, ctx);
        response = await logout(request, env, auth);
...
async function readJson(request) {
  ...
  try {
    ...
    fail(400, 'BAD_JSON', 'بيانات JSON غير صالحة.');
  }
}
...
function randomId(size = 24) {
  return bytesToB64Url(randomBytes(size));
}
...
async function activate(request, env) {
  ...
  if (license.expires_at && new Date(license.expires_at).getTime() < Date.now()) {
    fail(403, 'CODE_EXPIRED', 'انتهت صلاحية الكود.');
  }
  if (license.locked_until && new Date(license.locked_until).getTime() > Date.now()) {
    fail(423, 'CODE_LOCKED', 'الكود مقفل مؤقتاً بسبب محاولات فاشلة.');
  }
  ...
  if (!license.device_fingerprint) {
    await env.DB.prepare(`
      UPDATE licenses
      SET student_name=?,device_key_jwk=?,device_fingerprint=?,activated_at=?,updated_at=?
      WHERE id=?
    `).bind(name, JSON.stringify(publicKey), fingerprint, nowIso(), nowIso(), license.id).run();
  } else if (!license.student_name) {
    await env.DB.prepare('UPDATE licenses SET student_name=?,updated_at=? WHERE id=?')
      .bind(name, nowIso(), license.id).run();
  }

  await env.DB.prepare('DELETE FROM challenges WHERE license_id=? OR expires_at<?')
    .bind(license.id, nowIso()).run();

  const challengeId = randomId(18);
  const nonce = randomId(32);

  await env.DB.prepare(`
    INSERT INTO challenges(id,license_id,nonce,expires_at,used,created_at)
    VALUES(?,?,?,?,0,?)
...
async function verifyChallenge(request, env) {
  ...
  if (!challengeId || !signature) {
    fail(400, 'MISSING_SIGNATURE', 'بيانات التحقق ناقصة.');
  }

  const row = await env.DB.prepare(`
    SELECT c.*,l.status,l.student_name,l.device_key_jwk,l.code_hint,l.failed_attempts,l.locked_until
    FROM challenges c
  ...
  if (new Date(row.expires_at).getTime() < Date.now()) {
    fail(401, 'CHALLENGE_EXPIRED', 'انتهت مهلة التحقق.');
  }
  if (row.status !== 'active') fail(403, 'CODE_DISABLED', 'الكود موقوف.');
  if (row.locked_until && new Date(row.locked_until).getTime() > Date.now()) {
    fail(423, 'CODE_LOCKED', 'الكود مقفل مؤقتاً.');
  }

  const publicKey = await crypto.subtle.importKey(
    'jwk',
    JSON.parse(row.device_key_jwk),
  ...
  const ok = await crypto.subtle.verify(
    ...
    encoder.encode(`${challengeId}.${row.nonce}`),
  );
  ...
  if (!ok) {
    ...
    await env.DB.prepare(`
      UPDATE licenses SET failed_attempts=?,locked_until=?,updated_at=? WHERE id=?
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(`
      ...
      WHERE id=?
    `).bind(loginAt, loginAt, row.license_id),
  ...
  await env.DB.prepare(`
    INSERT INTO sessions(token_hash,license_id,expires_at,created_at,last_seen_at,ip_hash,ua_hash)
    VALUES(?,?,?,?,?,?,?)
  `).bind(hash, row.license_id, expiresAt, loginAt, loginAt, ipHash, uaHash).run();
...
async function requireStudent(request, env, ctx) {
  ...
  const hash = await tokenHash(token, env);
  const row = await env.DB.prepare(`
    SELECT s.token_hash,s.expires_at,s.license_id,
           l.student_name,l.code_hint,l.status,l.device_fingerprint,l.device_key_jwk
    FROM sessions s
  ...
  const seen = await env.DB.prepare('SELECT nonce_hash FROM request_nonces WHERE nonce_hash=?')
    .bind(nonceHash).first();

  if (seen) fail(409, 'REPLAY_BLOCKED', 'تم رفض طلب مكرر.');

  await env.DB.prepare(`
    INSERT INTO request_nonces(nonce_hash,license_id,expires_at) VALUES(?,?,?)
  `).bind(nonceHash, row.license_id, futureIso(5 * 60 * 1000)).run();
...
function publicStudent(auth) {
  return {
    name: auth.student_name || 'طالب',
    codeHint: auth.code_hint,
    deviceFingerprint: auth.device_fingerprint,
  };
}
...
async function logout(request, env, auth) {
  ...
  await env.DB.batch([
    env.DB.prepare(`
      INSERT INTO student_activity(license_id,last_logout_at,updated_at)
      VALUES(?,?,?)
      ON CONFLICT(license_id) DO UPDATE SET
        last_logout_at=excluded.last_logout_at,
        updated_at=excluded.updated_at
    `).bind(auth.license_id, logoutAt, logoutAt),
    env.DB.prepare('DELETE FROM sessions WHERE token_hash=?').bind(hash),
  ]);

  return json({ ok: true, loggedOutAt: logoutAt });
}
...
async function listChapters(env, auth) {
  ...
  return json({
    ...
    chapters: (rows.results || []).map(row => ({
      id: Number(row.chapter),
      title: titles[row.chapter]?.[0] || `الفصل ${row.chapter}`,
      description: titles[row.chapter]?.[1] || '',
...
async function listQuestions(url, env) {
  ...
  let sql = `
    SELECT id,chapter,topic,difficulty,type,text,options_json,source
    FROM questions
    WHERE active=1 AND chapter=?
  `;
  const binds = [chapter];
...
async function submitAnswers(request, env, auth) {
  ...
  const rows = await env.DB.prepare(`
    SELECT id,chapter,correct_index,explanation
    FROM questions
  ...
  const statements = [
    env.DB.prepare(`
      INSERT INTO attempts(id,license_id,chapter,score,total,answers_json,created_at)
      VALUES(?,?,?,?,?,?,?)
    `).bind(
      attemptId,
      auth.license_id,
    ...
    statements.push(
      env.DB.prepare(`
        INSERT INTO student_question_progress(
          license_id,question_id,first_answered_at,last_answered_at,times_answered,best_correct
        )
        VALUES(?,?,?,?,1,?)
        ON CONFLICT(license_id,question_id) DO UPDATE SET
          last_answered_at=excluded.last_answered_at,
          times_answered=student_question_progress.times_answered+1,
          best_correct=CASE
            WHEN excluded.best_correct>student_question_progress.best_correct
            THEN excluded.best_correct
            ELSE student_question_progress.best_correct
          END
      `).bind(
        auth.license_id,
        result.id,
...
async function adminLogin(request, env) {
  ...
  if (!env.ADMIN_PASSWORD || !env.SESSION_SECRET) {
    fail(500, 'MISSING_SECRET', 'أسرار الإدارة غير مضبوطة.');
  }
  ...
  if (expected !== supplied) {
    fail(401, 'ADMIN_LOGIN_FAILED', 'كلمة مرور الإدارة غير صحيحة.');
  }

  const token = randomId(32);
  const expiresAt = futureIso(4 * 60 * 60 * 1000);

  await env.DB.prepare(`
    INSERT INTO admin_sessions(token_hash,expires_at,created_at) VALUES(?,?,?)
  `).bind(await tokenHash(token, env), expiresAt, nowIso()).run();
...
async function requireAdmin(request, env) {
  ...
  if (!row || new Date(row.expires_at).getTime() < Date.now()) {
    fail(401, 'ADMIN_SESSION_EXPIRED', 'انتهت جلسة الإدارة.');
  }
...
async function createCodes(request, env) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    await env.DB.prepare(`
      INSERT INTO licenses(
        code_hash,code_hint,label,status,expires_at,failed_attempts,created_at,updated_at
      )
      VALUES(?,?,?,'active',?,0,?,?)
    `).bind(hash, code.slice(-4), label, expiresAt, nowIso(), nowIso()).run();
  ...
  return json({
    codes,
    warning: 'تظهر الأكواد الكاملة مرة واحدة فقط. خزّنها في مكان آمن.',
  }, 201);
}
...
async function listCodes(url, env) {
  ...
  const rows = await env.DB.prepare(`
    ...
    SELECT
      ...
      CASE
        ...
        THEN 1 ELSE 0
      END AS is_online,
      COALESCE(p.unique_questions,0) AS unique_questions,
      COALESCE(p.best_correct,0) AS best_correct,
      COALESCE(t.attempts_count,0) AS attempts_count,
  ...
  const licenses = (rows.results || []).map(row => ({
    ...
    unique_questions: Number(row.unique_questions || 0),
    attempts_count: Number(row.attempts_count || 0),
    progress_percent: totalQuestions
      ? Math.min(100, Math.round(Number(row.unique_questions || 0) * 100 / totalQuestions))
      : 0,
    average_score: Number(row.answer_total || 0)
      ? Math.round(Number(row.correct_total || 0) * 100 / Number(row.answer_total || 0))
      : 0,
  }));
...
async function resetCode(id, env) {
  await env.DB.batch([
    ...
    env.DB.prepare('DELETE FROM request_nonces WHERE license_id=?').bind(id),
    env.DB.prepare('DELETE FROM attempts WHERE license_id=?').bind(id),
    env.DB.prepare('DELETE FROM student_question_progress WHERE license_id=?').bind(id),
    env.DB.prepare('DELETE FROM student_activity WHERE license_id=?').bind(id),
  ]);
...
async function toggleCode(id, env) {
  ...
  if (next === 'disabled') {
    await env.DB.prepare('DELETE FROM sessions WHERE license_id=?').bind(id).run();
  }
...
async function adminStats(env) {
  ...
  const [licenses, active, bound, questions, attempts, online] = await Promise.all([
    env.DB.prepare('SELECT COUNT(*) AS c FROM licenses').first(),
    env.DB.prepare("SELECT COUNT(*) AS c FROM licenses WHERE status='active'").first(),
    env.DB.prepare('SELECT COUNT(*) AS c FROM licenses WHERE device_fingerprint IS NOT NULL').first(),
    env.DB.prepare('SELECT COUNT(*) AS c FROM questions WHERE active=1').first(),
    env.DB.prepare('SELECT COUNT(*) AS c FROM attempts').first(),
    env.DB.prepare(`
      SELECT COUNT(DISTINCT s.license_id) AS c
      FROM sessions s
      JOIN licenses l ON l.id=s.license_id
      WHERE s.last_seen_at>=? AND s.expires_at>? AND l.status='active'
    `).bind(cutoff, now).first(),
  ]);

  return json({
    totalCodes: Number(licenses?.c || 0),
    activeCodes: Number(active?.c || 0),
    boundDevices: Number(bound?.c || 0),
    questions: Number(questions?.c || 0),
    attempts: Number(attempts?.c || 0),
    onlineStudents: Number(online?.c || 0),
  });
}
...
async function seedQuestions(request, env) {
  ...
  if (!bank || !Array.isArray(bank.questions)) {
    fail(500, 'BAD_BANK', 'ملف بنك الأسئلة غير صالح.');
  }

  const statements = bank.questions.map(question => env.DB.prepare(`
    INSERT INTO questions(
      id,chapter,topic,difficulty,type,text,options_json,
      correct_index,explanation,source,active,updated_at
    )
    VALUES(?,?,?,?,?,?,?,?,?,?,1,?)
